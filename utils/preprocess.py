...
class Preprocess:
    ...
    def process(self, image):
        ...
        face = self.cropFace(image_align, landmarks_align)
        mask = self.segment.get_mask(face)
        return np.dstack((face, mask))

    @staticmethod
    def cropFace(image, landmarks):
        ...
        image_cropped = np.ones((bottom - top + 1, right - left + 1, 3), np.uint8) * 255
        ...
        image_cropped[top_white:bottom_white+1, left_white:right_white+1] = image[top:bottom+1, left:right+1].copy()
